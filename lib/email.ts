...
export async function sendTransactionalEmail(input: { userId: string; orderId?: string; template: "payment_approved" | "payment_pending" | "payment_rejected" | "account_recovery" }) {
  ...
  try {
    ...
    const result = await resend.emails.send({ from: process.env.RESEND_FROM_EMAIL ?? "Luana Sucesso <onboarding@resend.dev>", to: profile.email, subject: subjects[input.template], html: `<p>Olá, ${profile.name || "professora"}.</p><p>${messages[input.template]}</p>` });
