...
function mapProduct(row: RawProduct): CatalogProduct {
  const category =
    row.product_categories?.[0]?.categories ?? null;

  const regular = Number(row.regular_price_cents);

  const sale =
    row.sale_price_cents === null ||
    row.sale_price_cents === undefined
      ? null
  ...
  return {
    ...
    salePriceCents: sale,
    effectivePriceCents: sale ?? regular,

    productKind:
      row.product_kind === "kit"
        ? "kit"
        : "individual",
    ...
    hotmart_offer_code: row.hotmart_offer_code
      ? String(row.hotmart_offer_code)
      : null,
  };
}
...
export async function listCatalog(
  ...
  const products = await Promise.all(
    ((data ?? []) as RawProduct[]).map(async (row) => {
  ...
  return products.sort((a, b) =>
    input.sort === "price_asc"
      ? a.effectivePriceCents -
        b.effectivePriceCents
...
export async function getProduct(slug: string) {
  ...
  const previewRows =
    (data.product_previews ?? []) as Array<{
      storage_path: string;
  ...
  return {
    product: {
      ...product,
      coverUrl:
        coverSigned?.signedUrl ?? null,
    },

    previews,

    kitItems:
      (data.kit_items ?? []) as Array<{
